...
Інструменти типу SIEM генерують десятки альортів та логують сотні тисяч записів на день. Різниця між подією та інцидентом критична, бо ці дві речі мають різні рівні пріоритету.  
...
Попри мислення як детективи, ми також маємо мислити як зловмисники. Які способи проникнення у нашу мережу ми бачимо, які сліпі зони чи непропатчені, забуті системи ми маємо, які дані нашої компанії є найкритичнішими для нас, які вірогідні цілі для атаки матимуть хакери, тощо. Для того щоб краще розуміти життєвий цикл кібератаки та знати як реагувати на конкретні фази цього цилку варто згадати [MITRE Attack Matrix](https://attack.mitre.org/) та [Cyber Kill Chain](https://www.microsoft.com/en-us/security/business/security-101/what-is-cyber-kill-chain#Eight-phases).

### 1.4 Проблеми, які вже вирішили за вас
Ти не перший і не останній кібербезпечник, якому треба побудувати IR екосистему у своїй компанії. Замість того щоб вигадувати колесо, варто спиратись на загальноприйняті стандарти, чи фреймворки. Наприклад:  
...
Розслідування інцидентів - це великий і важкий процес який складається з різних етапів, і якість підготовки та виконання цих етапів визначить наскільки ефективно ви зможете детектувати, розслідувати, та зупиняти кібератаки у своїй компанії.  
...
На відміну від всіх наступних етапів, підготовка це постійні активності, покращення, планування, та навчання, які робляться до інциденту. Це найважливіший етап, бо без нього навіть найкращі аналітики не дадуть гарного результату в розслідуванні інцидентів.  

Коли станеться інцидент, які інструменти це задетектують, або як саме працівники компанії повідомлять тебе про підозрілу активність? Як саме ти розслідуватимеш цю подію, кому ти повідомиш про інцидент та хто з твоєї компанії буде частиною CSIRT команди? Які інструменти для кожного з етапів IR ти маєш використовувати? Який у вас план дій по відновленню нормальної роботи бізнесу? Чи є у вас бекапи, де вони, та як саме відновитись з цих бекапів? Що робити, якщо вся корпоративна мережа комунікації перестане працювати, чи ти знаєш номери телефонів членів CSIRT? Якщо корпоративні компʼютері стануть недоступними, які компʼютери використовуватимуть аналітики, як ці компʼютери отримають доступ до корпоративної мережі? Таким питанням немає кінця, але відповідати на них - це і є ціль фази підготовки до IR, і саме через це ця фаза постійна і ніколи не закінчується.  

Програма-мінімум яку варто виконати в цій фазі:  
1. Incident Response Policy. Політика, перевірена та затверджена менеджментом компанії, має як мінімум описувати ролі та відповідальності різних посад у компанії та їх дії під час інциденту (з кого складається команда CSIRT), формальні визначення понять інциденту та інших термінів для цієї конкретної компанії, план та деталі деталі публічних оголошень про інциденти (особливо коли це вимагається законом чи комплаєнсом), як визначати severity інцидентів та пріорітизовувати їх, які KPI вимірювати, шляхи повідомлення кібербезпечників про інциденти для звичайних працівників, та багато іншого. [Приклад Incident Response Policy](https://frsecure.com/incident-response-policy-template/)  
...
3. Якщо система виконує функції які не можна переривати (наприклад є бекендом застосунку для клієнтів), то підняти альтернативну систему з таким самим функціоналом, та перенаправити весь трафік на нову систему (зазвичай за допомогою зміни DNS запису).  Саме для таких обставин інфраструктура має бути у вигляді IaC, щоб скоротити час підняття та мінімізувати ручні налаштування.  
4. Заблокувати можливість підключатись до корпоративної мережі з інфікованого пристрою (наприклад при використанні VPN заблокувати інфікованого користувача, його credentials, сертифікати, тощо).
...
Після видалення зарази необхідно відновити нормальну роботу систем які були інфіковані. Звичайно в ідеальному випадку у вас мають бути бекапи для всіх даних які є критично важливими і які не можна втратити. Чим краще автоматизація систем - тим простіше буде відновлюватись. Для поширених невеликих інцидентів recovery буде відносно простим та інтуїтивним. Для більших інцидентів у вас гарантовано будуть досвідчені люди що керуватимуть процесом та роздаватимуть задачі. Після відновлення роботи варто також якийсь час моніторити активність раніше заражених систем щоб впевнитись що інфекція була повністю знищена і не повернулась.  

### 2.6 Lessons learned (Активності після інциденту та покращення IR процесів)
Останній етап потрібен для постійного покращення та відшліфування процесу IR. Це робиться під час написання Incident Report. [Приклад IR Report від HackTheBox](https://docs.google.com/document/d/e/2PACX-1vTkpIEicOwy8tRNAxKa4f4cMpc7wrUHRM5Nr_A_bObpzDQg5pAvj7jniHa_uVWQCuHLr21hQO64BsVi/pub). Під час підготовчої фази ваша команда має створити теплейт для IR Report. У репорті має міститись як мінімум наступна інформація:  
1. Унікальний ID інциденту
2. Severity інциденту
3. Хто розслідував інцидент
4. Impact  
5. Хронологія інциденту
6. Key findings - ключові докази  
7. Які дії були зроблені CISRT командою  
8. Першопричина (що зробило інцидент можливим)  
9. Відповідальні особи за причини інциденту (якщо такі є)  
11. Що можна було б покращити щоб уникнути подібного інциденту у майбутньому
12. Технічні деталі інциденту з усіма доказами (Якщо є можливість)  

Коли ви дістанетесь Post-Incident Activity, ви маєте зрозуміти які саме були проблеми та помилки, що саме могло піти краще. Найпоширенішими помилками є наступні:  
...
4. Не було зроблено бекапів систем, або бекапи знаходились на тій самій системі з якої робились, відповідно були теж інфікованими.
...
10. CSIRT команда не знала що в них в принципі є план та плейбуки, всі діяли "інтуїтивно".
...
## 3. Висновки
IR - це неймовірно цікава активність, яка вимагає тренувань та підготовки. Чи краще підготовка - тим швидше і краще будуть знищені інфекції.  Матеріалу вище має бути достатньо щоб почати розслідувати прості інциденти, але реальні глибокі знання та розуміння як краще прийдуть тільки з досвідом.  
